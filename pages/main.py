...
# Apply custom CSS
def apply_css():
    css = load_html("styles.css")
    st.markdown(f"<style>{css}</style>", unsafe_allow_html=True)

# Display HTML content
def display_html(file_name):
    html = load_html(file_name)
...
def main_page():
    ...
    with main_container:
        ...
        stock_input = st.text_input("Enter the Stock ID", "GOOG", key="stock_input")
        search_button = st.button("Search")

        # Date range selection
        default_start_date = datetime(2015, 3, 2)
        end = datetime.now()
        ...
        if search_button and stock_input:
            ...
            for recent_stock in st.session_state.recent_stocks:
                if st.sidebar.button(f"Load {recent_stock}"):
                    ...
                    st.rerun()  # Refresh to show the selected recent stock
            ...
            plt.plot(plotting_data.index, plotting_data['original_test_data'], label='Original Data', color='blue')
            plt.plot(plotting_data.index, plotting_data['predictions'], label='Predicted Data', color='orange')
            plt.xlabel('Date')
            plt.ylabel('Price')
